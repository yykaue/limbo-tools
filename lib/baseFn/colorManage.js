...
function rgbToHex (rgbArr) {
  let hex = ((1 << 24) + (rgbArr[0] << 16) + (rgbArr[1] << 8) + rgbArr[2]).toString(16).slice(1)
  if (rgbArr.length > 3) {
 ...
 * @param endColor
 * @param steps
 * @param colorType 输出类型 ['hex', 'rgb']
 * @returns {[]}
 */
function gradientColors (startColor, endColor = '#fff', steps = 11, colorType = 'hex') {
  const colorList = []
  let sColor, eColor
